...
UPDATE_PARAMS = ('name', 'description','status','progress','config_set_id',
                 ...
                 'deleted', 'db_lv_size', 'nova_lv_size')

CREATE_PARAMS = ('id', 'name','description','status','progress','config_set_id',
                 'nodes','services','cluster_id','type','vip', 'glance_lv_size',
				 'deployment_backend',  'db_lv_size', 'nova_lv_size')
...
class RoleManager(base.ManagerWithFind):
    ...
    def _role_meta_from_headers(self, headers):
        meta = {'properties': {}}
    ...
    @staticmethod
    def _format_role_meta_for_user(meta):
        for key in ['size', 'min_ram', 'min_disk']:
            if key in meta:
                try:
                    meta[key] = int(meta[key]) if meta[key] else 0
    ...
    def get(self, role, **kwargs):
        """Get the metadata for a specific role.

        :param role: host object or id to look up
        :rtype: :class:`Role`
        """
    ...
    def list(self, **kwargs):
        ...
        :param marker: begin returning roles that appear later in the role
                       list than that represented by this role id
        :param filters: dict of direct comparison filters that mimics the
                        structure of an role object
        ...
        def paginate(qp, return_request_id=None):
            ...
            url = '/v1/roles/detail?%s' % urlparse.urlencode(qp)
            roles, resp = self._list(url, "roles")
        ...
        while True:
            seen_last_page = 0
            filtered = 0
            for role in paginate(params, return_request_id):
                last_role = role.id
            ...
            if seen_last_page + filtered == 0:
                # Note(kragniz): we didn't get any roles in the last page
                return

            if absolute_limit is not None and seen >= absolute_limit:
                # Note(kragniz): reached the limit of roles to return
                return

            if page_size and seen_last_page + filtered < page_size:
                # Note(kragniz): we've reached the last page of the roles
                return

            # Note(kragniz): there are more roles to come
            params['marker'] = last_role
            seen_last_page = 0
    ...
    def delete(self, role, **kwargs):
        """Delete an role."""
        url = "/v1/roles/%s" % base.getid(role)
    ...
    def update(self, role, **kwargs):
        ...
        hdrs.update(self._role_meta_to_headers(fields))
        ...
        return Role(self, self._format_role_meta_for_user(body['role_meta']))
